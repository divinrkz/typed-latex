...
let generate_match_id (pattern_case : string) =
  ...
  | "TypeName" ->
      incr id_counters.type_name;
      !(id_counters.type_name)
  | "DefContainer" ->
      incr id_counters.def_container;
      !(id_counters.def_container)
  | "Relation" ->
      incr id_counters.relation;
      !(id_counters.relation)
  | "Expression" ->
      incr id_counters.expression;
...
let parse_patterns filename =
  In_channel.with_file filename ~f:(fun input_c ->
      ...
      In_channel.iter_lines input_c ~f:(fun line ->
          ...
          | Some first_split -> (
              process_first_split first_split seq;
              ...
              | Some _ -> (
                  ...
                  | None ->
                      print_endline
                        ...
                        ^ ": No first split found.")
                  | Some third_split ->
                      process_first_split third_split seq;
