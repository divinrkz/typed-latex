...
module PatternDef : sig
  ...
  type t = { id : int; line : string; pattern : pattern }
...
module PatternExtractor = struct
  ...
  let parse_math_pattern s =
    if Util.String.starts_with s "M" then
      ...
      MathPattern (Expression (MatchID.from_int 1))
    else failwith "Invalid math pattern"
  ...
  let rec parsing_pattern s =
    ...
    if String.length s > 1 && Util.String.starts_with s "M" then
      parse_def_container (parse_math_pattern s) (MatchID.from_int 1)
      ...
      parse_sequence (List.map ~f:parsing_pattern parts) (* Case: Simple word *)
    else parse_word s
  ...
  let parse_relations relations_str =
              ...
              ( MathPattern
                  (Function
                     ( parse_relation_type relation,
                       [ Expression (gen_id ()); Expression (gen_id ()) ],
                       gen_id () )),
                gen_id () )
          in
    ...
    match relations with _ -> [ Any relations ]
  ...
  let parse_words words_str seq =
    ...
    let rec process_words words acc =
      ...
      | str1 :: str2 :: rest ->
          let stripped_str1 =
            if str_ends_with str1 "|" then
              String.sub str1 ~pos:0 ~len:(String.length str1 - 1)
            else str1
          in
          let stripped_str2 =
            if str_ends_with str2 "|" then
              String.sub str2 ~pos:0 ~len:(String.length str2 - 1)
            else str2
          in
          if str_ends_with str1 "|" || str_ends_with str2 "|" then
            acc
            @ [ Any [ Word stripped_str1; Word stripped_str2 ] ]
            @ process_words rest []
          else acc @ [ Word str1 ] @ process_words (str2 :: rest) []
    in
    let rec parse_word splits acc =
      ...
      | word :: rest ->
          ...
          let new_acc =
            ...
            | [ opt ] ->
                let stripped_opt =
                  Util.regex_first_matcher opt regex_first_split
                in
  ...
  let extract_first_split first_split =
    ...
    let rec parse_word word =
      ...
      let word =
        if has_dollar then String.sub ~pos:0 ~len:(String.length word - 1) word
        else word
      in
      let is_optional = str_ends_with word "?" in
      let word =
        if is_optional then String.sub ~pos:0 ~len:(String.length word - 1) word
        else word
      in
      let word =
      ...
      if String.contains word '|' then
        ...
        let any_list =
          List.filter
            ~f:(function Word _ -> true | _ -> false)
            (any_of_list parts)
        in
        if is_optional then Optional (Any any_list) else Any any_list
      else
        ...
        if is_valid_word word then
          if is_optional then Optional (Word word) else Word word
        else Sequence []
    in
  ...
  *)
  let process_first_split first_split seq = parse_words first_split seq
    ...
    In_channel.with_file filename ~f:(fun input_c ->
        ...
        In_channel.iter_lines input_c ~f:(fun line ->
            ...
            | None ->
                print_endline
                  ("Line "
                  ^ string_of_int !line_counter
                  ^ ": No first split found.")
            | Some first_split ->
                let parsed = extract_first_split first_split in
                seq := !seq @ parsed);
            ...
            | None ->
                print_endline
                  ("Line "
                  ^ string_of_int !line_counter
                  ^ ": No third split found.")
            | Some third_split -> process_first_split third_split seq);

            let extracted =
              PatternDef.constr (!line_counter, line, Sequence !seq)
            in
            add_pattern extractor extracted))
end
