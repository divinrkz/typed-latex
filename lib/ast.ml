...
module rec Math : sig
  ...
  type unary = Negate | Not | Abs
  type logic_op = And | Or
  type logic = Implies | Iff
  ...
  and t =
    ...
    | Grouping of t
    | Apply of t * t list
      (* ex: f(args) (if args has length 1, could also be interpreted as multiplication) *)
    | Subscript of t * t
      (* not implemented as an binop since can be interpreted in different ways depending on context *)
    | Superscript of t * t
  ...
  let string_of_unary = function Negate -> "-" | Not -> "NOT" | Abs -> "ABS"
  ...
  let string_of_logic_op = function And -> "AND" | Or -> "OR"
  let string_of_logic = function Implies -> "IMPLIES" | Iff -> "IFF"
  ...
  let is_greek_letter name =
    match name with
    | "\\alpha" | "\\nu" | "\\beta" | "\\xi" | "\\Xi" | "\\gamma" | "\\Gamma"
    | "\\delta" | "\\Delta" | "\\pi" | "\\Pi" | "\\epsilon" | "\\varepsilon"
    | "\\rho" | "\\varrho" | "\\zeta" | "\\sigma" | "\\Sigma" | "\\eta"
    | "\\tau" | "\\theta" | "\\vartheta" | "\\Theta" | "\\upsilon" | "\\Upsilon"
    | "\\iota" | "\\phi" | "\\varphi" | "\\Phi" | "\\kappa" | "\\chi"
    | "\\lambda" | "\\Lambda" | "\\psi" | "\\Psi" | "\\mu" | "\\omega"
    | "\\Omega" ->
        true
    | _ -> false

  let rec pp_relation formatter rel =
    match rel with
  ...
  and pp formatter math =
    match math with
    | Op (lhs, op, rhs) ->
        Format.fprintf formatter "(%s %a %a)" (string_of_operator op) pp lhs pp
          rhs
    | LogicOp (lhs, op, rhs) ->
        Format.fprintf formatter "(%s %a %a)" (string_of_logic_op op) pp lhs pp
          rhs
    | Logic (lhs, rhs) ->
        let pp_pair formatter (rel, t) =
          Format.fprintf formatter "%s %a" (string_of_logic rel) pp t
        in
        Format.fprintf formatter "(%a %a)" pp lhs
          (Format.pp_print_list ~pp_sep:(string_sep " ") pp_pair)
          (List.rev rhs)
    | Unary (op, lhs) ->
        Format.fprintf formatter "(%s %a)" (string_of_unary op) pp lhs
    ...
    | Relation (lhs, rhs) ->
        let pp_pair formatter (rel, t) =
          Format.fprintf formatter "%a %a" pp_relation rel pp t
        in
        Format.fprintf formatter "(%a %a)" pp lhs
          (Format.pp_print_list ~pp_sep:(string_sep " ") pp_pair)
          (List.rev rhs)
    | Apply (lhs, rhs) ->
        Format.fprintf formatter "(%a %a)" pp lhs
          (Format.pp_print_list ~pp_sep:(string_sep " ") pp)
          rhs
    | Subscript (lhs, rhs) -> Format.fprintf formatter "%a_%a" pp lhs pp rhs
    | Superscript (lhs, rhs) ->
        Format.fprintf formatter "(^ %a %a)" pp lhs pp rhs
    | Command (name, arg) -> (
        match arg with
        | Some thing -> Format.fprintf formatter "%s{%a}" name pp thing
        | None -> Format.fprintf formatter "%s" name)
    | Forall (expr, next) ->
        Format.fprintf formatter "(FORALL %a, %a)" pp expr pp next
    | Exists (expr, next) ->
        Format.fprintf formatter "(EXISTS %a %a)" pp expr pp next
    ...
    | Summation (low, high, body) ->
        Format.fprintf formatter "(SUM_%a^%a %a)" pp low pp high pp body
    | Tuple contents ->
        Format.fprintf formatter "(%a)"
          (Format.pp_print_list ~pp_sep:(string_sep ", ") pp)
          contents
  ...
  type env = Env of (string, Typing.Var.t) Hashtbl.t * env | Empty

  let new_env parent = Env (Hashtbl.create (module String), parent)

  let add_var env name data =
    match env with
    | Env (vars, _) -> Hashtbl.set vars ~key:name ~data
    | Empty ->
        raise (TypeError "Empty environment (this should not ever happen)")
  ...
  let add_fn (env : fns) name data = Hashtbl.set env ~key:name ~data
  ...
  let type_check (math : t list) =
    ...
    let recurse_var env (name : string) =
      ...
      | None ->
          ...
          (* free variables go straight to the top level environment *)
          Type.Any t
    in
    ...
    let rec captured_vars node =
      ...
      | Relation (lhs, _) -> captured_vars lhs
      | _ -> []
    in

    let capture parent node =
      let vars = captured_vars node in
      if List.length vars = 0 then parent
      else
    ...
    let rec recurse_fn env name args =
      ...
      | Some (args_t, return_t) ->
          if List.length args <> List.length args_t then
            raise
              (TypeError
                 (Format.sprintf "Conflicting declarations for function %s" name))
          else
            List.iter (List.zip_exn args args_t) ~f:(fun (x, y) ->
                add_constraint (recurse env x, Type.Any y));
          (args_t, return_t)
      | None ->
    ...
    and recurse (env : env) node =
      ...
      | Variable name -> recurse_var env name
      | Grouping expr -> recurse env expr
      | Relation (lhs, rhs) ->
          ...
          let verify first arr =
            ...
            let (seen : state) =
              { le = false; ge = false; sub = false; sup = false }
            in
            ...
            let rec iter prev_t arr =
              ...
              | hd :: tl -> (
                  match hd with
                  | (Le, _ | Leq, _) when seen.ge ->
                      raise (TypeError "> and >= should be followed by < or <=")
                  | Le, expr | Leq, expr ->
                      seen.le <- true;
                  ...
                  | Ge, expr | Geq, expr ->
                      ...
                      (* add_constraint (t, Type.Number Natural); *)
                      iter t tl
                  | (Superset, _ | SupersetEq, _) when seen.sub ->
                      raise
                        (TypeError
                           "Subset(eq) should not be followed by superset(eq)")
                  | Superset, expr | SupersetEq, expr ->
                      ...
                      iter t tl
                  | (Subset, _ | SubsetEq, _) when seen.sup ->
                      raise
                        (TypeError
                           "Superset(eq) should not be followed by subset(eq)")
      ...
      | Command (name, arg) -> (
          ...
          | "\\sqrt", Some lhs ->
              ...
              Type.Any t
          | "\\sqrt", None ->
              raise (TypeError "sqrt not provided with an argument")
          | _ ->
              Type.Any (Typing.Var.fresh ())
    ...
    let rec all_vars arr =
      match arr with
      | [] -> true
      | hd :: tl -> ( match hd with Variable _ -> all_vars tl | _ -> false)
    in
          ...
          if all_vars args then (
            ...
            List.iter args ~f:(fun arg ->
                ...
                | _ ->
                    raise
                      (TypeError
                         "Invalid function declaration (this should not happen)"));
            let _ = recurse_fn child name args in
            recurse child node)
          else recurse env node
      | _ -> recurse env node
    in

    List.iter
      ~f:(fun x ->
        let _ = recurse0 top_level x in
        ())
      math;
    ...
    | Env (vars, _) ->
        Hashtbl.iteri vars ~f:(fun ~key ~data ->
            let principal_type =
              Typing.simplify ~final:true (Typing.apply subs (Type.Any data))
            in
            Format.printf "val (%a) %s : %a\n" Typing.Var.pp data key Type.pp
              principal_type);

        Hashtbl.iteri functions ~f:(fun ~key ~data ->
            let args_t, ret_t = data in
            let arg_types =
              List.map args_t ~f:(fun a ->
                  Typing.simplify ~final:true (Typing.apply subs (Type.Any a)))
            in
            let return_type =
              Typing.simplify ~final:true (Typing.apply subs (Type.Any ret_t))
            in
            Format.printf "fun %s : %a -> %a\n" key
              (Format.pp_print_list ~pp_sep:(string_sep " -> ") Type.pp)
              arg_types Type.pp return_type)
    | Empty ->
        ();
...
and Statement : sig
  type t = Fact of Math.t | Assumption of Math.t | Definition of Math.t
...
module Node = struct
  type 'node t = { pos : Lexing.position; value : 'node }
end
...
and Latex : sig
  ...
  and pp formatter (latex : t) =
    ...
    | { pos = _; value = Command (name, args) } ->
        Format.fprintf formatter "(\\%s %a)" name
          (Format.pp_print_list ~pp_sep:(string_sep " ") pp)
          args
    | { pos = _; value = Latex contents } ->
        Format.fprintf formatter "(%a)" (Format.pp_print_list pp) contents

  let pp_list formatter asts =
    Format.fprintf formatter "%a\n"
  ...
  let get_all_math node =
    let acc = ref [] in
    let rec recurse acc (node : t) =
      match node with
      | { pos = _; value = Word _ } -> ()
      | { pos = _; value = Whitespace } -> ()
      | { pos = _; value = Newline } -> ()
      | { pos = _; value = Environment (_, _, contents) } ->
          List.iter ~f:(recurse acc) contents
      | { pos = _; value = Mathmode _ as math } -> acc := math :: !acc
      | { pos = _; value = Multiline _ as math } -> acc := math :: !acc
      | { pos = _; value = Command (_, args) } ->
          List.iter ~f:(recurse acc) args
      | { pos = _; value = Latex contents } ->
          List.iter ~f:(recurse acc) contents
    in
    recurse acc node;
